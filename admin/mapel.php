...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'tambah_multi') {
        ...
        foreach ($names as $nama) {
            ...
            $row = db()->query("SELECT id FROM mapel ORDER BY CAST(SUBSTRING(id,2) AS UNSIGNED) DESC LIMIT 1")->fetch();
            $nextNum = $row ? (int)substr($row['id'], 1) + 1 : 1;
            $newId = 'M' . $nextNum;
            $stmt = db()->prepare("INSERT INTO mapel (id, nama_mapel) VALUES (?, ?)");
            $stmt->execute([$newId, $nama]);
            $inserted++;
        }
        echo json_encode(['success' => true, 'message' => "$inserted mapel berhasil ditambahkan"]);
        exit;
    }
    ...
    if ($action === 'edit') {
        ...
        echo json_encode(['success' => true, 'message' => 'Mapel berhasil diperbarui']);
        exit;
    }
    ...
    if ($action === 'hapus') {
        ...
        $stmt->execute([$id]);
        echo json_encode(['success' => true, 'message' => 'Mapel berhasil dihapus']);
        exit;
    }
    ...
    if ($action === 'hapus_multiple') {
        ...
        if (!empty($ids) && is_array($ids)) {
            ...
            db()->prepare("DELETE FROM mapel WHERE id IN ($placeholders)")->execute($ids);
        }
        echo json_encode(['success' => true, 'message' => 'Mapel terpilih berhasil dihapus']);
        exit;
    }
...
$mapelList = db()->query("SELECT * FROM mapel ORDER BY CAST(SUBSTRING(id,2) AS UNSIGNED)")->fetchAll();
...
<title>Menu Mapel - <?= htmlspecialchars($appName) ?></title>
...
<main class="ml-64 pt-16 pb-10 px-6">
  <div class="mt-6">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">Menu Mapel</h1>
      <button onclick="openAddModal()" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-medium transition">
    ...
    <div class="bg-white rounded-xl shadow overflow-hidden">
      <div class="p-4 border-b flex items-center justify-between gap-4 flex-wrap">
        ...
        <div class="flex items-center gap-3">
          ...
          <input type="text" id="search" oninput="filterTable()" placeholder="Cari mapel..."
            class="border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
        </div>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody id="mapel-tbody">
            ...
            <tr class="border-t hover:bg-gray-50 mapel-row"
                data-search="<?= htmlspecialchars(strtolower($mapel['nama_mapel'])) ?>">
              <td class="px-4 py-3">
                <input type="checkbox" class="row-check" value="<?= htmlspecialchars($mapel['id']) ?>" onchange="updateBulkBtn()">
              </td>
              <td class="px-4 py-3 font-mono font-bold text-purple-600"><?= htmlspecialchars($mapel['id']) ?></td>
              <td class="px-4 py-3 font-medium"><?= htmlspecialchars($mapel['nama_mapel']) ?></td>
              <td class="px-4 py-3 text-gray-500 text-xs"><?= htmlspecialchars($mapel['created_at'] ?? '') ?></td>
              <td class="px-4 py-3 text-center">
                <button onclick="editMapel('<?= htmlspecialchars($mapel['id']) ?>','<?= htmlspecialchars(addslashes($mapel['nama_mapel'])) ?>')"
                  class="text-blue-600 hover:text-blue-800 mr-2 p-1" title="Edit">
                  <i class="fas fa-edit"></i>
                </button>
                <button onclick="hapusMapel('<?= htmlspecialchars($mapel['id']) ?>')"
...
<div id="add-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
  <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg mx-4">
    <h3 class="text-lg font-bold text-gray-800 mb-4">Tambah Mapel</h3>
    <div id="rows-container">
      <div class="flex gap-2 mb-2">
        <input type="text" placeholder="Nama mata pelajaran (contoh: Matematika)"
          class="row-input flex-1 border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
        <button onclick="removeRow(this)" class="text-red-500 hover:text-red-700 px-2" title="Hapus baris">
          <i class="fas fa-times"></i>
        </button>
      </div>
    </div>
    <button onclick="addRow()" class="text-purple-600 hover:text-purple-800 text-sm mb-4">
      <i class="fas fa-plus mr-1"></i>Tambah Baris
    </button>
    <div class="flex gap-3 justify-end">
      <button onclick="closeAddModal()" class="px-4 py-2 border rounded-lg text-sm hover:bg-gray-50">Batal</button>
      <button onclick="submitTambah()" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-sm font-medium">Simpan</button>
    </div>
  </div>
</div>
...
<div id="edit-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
  <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4">
    <h3 class="text-lg font-bold text-gray-800 mb-4">Edit Mapel</h3>
    ...
    <div class="flex gap-3 justify-end">
      ...
      <button onclick="submitEdit()" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-sm font-medium">Simpan</button>
    </div>
  </div>
</div>
...
function renderPagination(total) {
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    btn.className = 'px-3 py-1 rounded text-sm ' +
      (i === currentPage ? 'bg-purple-600 text-white' : 'bg-gray-200 hover:bg-gray-300');
...
function openAddModal() {
  document.getElementById('rows-container').innerHTML =
    '<div class="flex gap-2 mb-2">' +
    '<input type="text" placeholder="Nama mata pelajaran (contoh: Matematika)" class="row-input flex-1 border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">' +
...
function addRow() {
  ...
  div.innerHTML =
    '<input type="text" placeholder="Nama mata pelajaran" class="row-input flex-1 border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">' +
    '<button onclick="removeRow(this)" class="text-red-500 hover:text-red-700 px-2"><i class="fas fa-times"></i></button>';
...
function submitTambah() {
  ...
  if (!names.length) { Swal.fire('Error', 'Nama mapel wajib diisi', 'error'); return; }

  const fd = new FormData();
  fd.append('csrf_token', csrf);
  fd.append('action', 'tambah_multi');
  names.forEach(n => fd.append('nama_mapel[]', n));

  fetch('mapel.php', { method: 'POST', body: fd })
    .then(r => r.json())
    .then(data => {
      if (data.success) {
...
function submitEdit() {
  ...
  if (!nama) { Swal.fire('Error', 'Nama mapel wajib diisi', 'error'); return; }

  const fd = new FormData();
  fd.append('csrf_token', csrf);
  fd.append('action', 'edit');
  fd.append('id', id);
  fd.append('nama_mapel', nama);
...
function hapusMapel(id) {
  Swal.fire({
    title: 'Hapus Mapel?', text: 'Data mapel akan dihapus permanen!', icon: 'warning',
...
function hapusTerpilih() {
  ...
  Swal.fire({
    title: `Hapus ${ids.length} mapel?`, text: 'Semua mapel terpilih akan dihapus permanen!', icon: 'warning',
    showCancelButton: true, confirmButtonColor: '#dc2626',
