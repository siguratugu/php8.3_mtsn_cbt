...
if (isset($_GET['action']) && $_GET['action'] === 'download_template') {
    ...
    fputcsv($out, ['Nama Siswa', 'NISN', 'ID Kelas']);
    fputcsv($out, ['Contoh Nama Siswa', '0012345678', 'K1']);
    fclose($out);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'tambah') {
        ...
        if (empty($nama) || empty($nisn)) {
            echo json_encode(['success' => false, 'message' => 'Nama dan NISN wajib diisi']);
            exit;
        }
        ...
        $dup = db()->prepare("SELECT id FROM siswa WHERE nisn = ?");
        $dup->execute([$nisn]);
        if ($dup->fetch()) {
        ...
        $stmt = db()->prepare(
            "INSERT INTO siswa (nama, nisn, kelas_id, password) VALUES (?, ?, ?, ?)"
        );
        $stmt->execute([$nama, $nisn, $kelasId ?: null, $password]);
    ...
    if ($action === 'edit') {
        ...
        $dup = db()->prepare("SELECT id FROM siswa WHERE nisn = ? AND id != ?");
        $dup->execute([$nisn, $id]);
        if ($dup->fetch()) {
            echo json_encode(['success' => false, 'message' => 'NISN sudah digunakan siswa lain']);
            exit;
        }
        db()->prepare("UPDATE siswa SET nama = ?, nisn = ?, kelas_id = ? WHERE id = ?")
            ->execute([$nama, $nisn, $kelasId ?: null, $id]);
    ...
    if ($action === 'hapus_multiple') {
        ...
        if (!empty($ids) && is_array($ids)) {
            ...
            db()->prepare("DELETE FROM siswa WHERE id IN ($placeholders)")->execute($ids);
        }
        echo json_encode(['success' => true, 'message' => 'Siswa terpilih berhasil dihapus']);
        exit;
    }
    ...
    if ($action === 'reset_password') {
        $id = (int)($_POST['id'] ?? 0);
        $password = password_hash('123456', PASSWORD_BCRYPT);
        db()->prepare("UPDATE siswa SET password = ? WHERE id = ?")->execute([$password, $id]);
    ...
    if ($action === 'reset_password_multiple') {
        ...
        if (!empty($ids) && is_array($ids)) {
            ...
            db()->prepare("UPDATE siswa SET password = ? WHERE id IN ($placeholders)")->execute($params);
        }
        echo json_encode(['success' => true, 'message' => 'Password siswa terpilih berhasil direset']);
        exit;
    }
    ...
    if ($action === 'import_csv') {
        ...
        $errors   = [];
        $password = password_hash('123456', PASSWORD_BCRYPT);
        $stmtCheck  = db()->prepare("SELECT id FROM siswa WHERE nisn = ?");
        $stmtKelas  = db()->prepare("SELECT id FROM kelas WHERE id = ?");
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            $nama    = trim($row[0]);
            $nisn    = trim($row[1]);
            $kelasId = isset($row[2]) ? trim($row[2]) : null;
            if (empty($nama) || empty($nisn)) { $skipped++; continue; }
            if (!preg_match('/^\d{10}$/', $nisn)) {
                $errors[] = "NISN tidak valid: $nisn";
                $skipped++;
                continue;
            }
            ...
            $stmtInsert->execute([$nama, $nisn, $validKelas, $password]);
            $inserted++;
        }
        fclose($handle);

        $msg = "$inserted siswa berhasil diimport";
...
<title>Menu Siswa - <?= htmlspecialchars($appName) ?></title>
...
<main class="ml-64 pt-16 pb-10 px-6">
  <div class="mt-6">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">Menu Siswa</h1>
    ...
    <div class="bg-white rounded-xl shadow overflow-hidden">
      <div class="p-4 border-b flex items-center justify-between gap-4 flex-wrap">
        ...
        <div class="flex items-center gap-3">
          ...
          <input type="text" id="search" oninput="filterTable()" placeholder="Cari nama, NISN, atau kelas..."
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody id="siswa-tbody">
            ...
            <tr class="border-t hover:bg-gray-50 siswa-row"
                data-search="<?= htmlspecialchars(strtolower($siswa['nama'] . ' ' . $siswa['nisn'] . ' ' . ($siswa['nama_kelas'] ?? ''))) ?>">
              <td class="px-4 py-3">
                <input type="checkbox" class="row-check" value="<?= (int)$siswa['id'] ?>" onchange="updateBulkBtn()">
              </td>
              <td class="px-4 py-3 row-no"><?= $i + 1 ?></td>
              <td class="px-4 py-3 font-medium"><?= htmlspecialchars($siswa['nama']) ?></td>
              <td class="px-4 py-3 font-mono text-gray-600"><?= htmlspecialchars($siswa['nisn']) ?></td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3 text-center">
                <button onclick="resetPassword(<?= (int)$siswa['id'] ?>, '<?= htmlspecialchars(addslashes($siswa['nama'])) ?>')"
                  class="text-yellow-600 hover:text-yellow-800 p-1 mr-1" title="Reset Password">
                  <i class="fas fa-key"></i>
                </button>
                <button onclick="editSiswa(<?= (int)$siswa['id'] ?>, '<?= htmlspecialchars(addslashes($siswa['nama'])) ?>', '<?= htmlspecialchars($siswa['nisn']) ?>', '<?= htmlspecialchars($siswa['kelas_id'] ?? '') ?>')"
                  class="text-blue-600 hover:text-blue-800 p-1 mr-1" title="Edit">
                  <i class="fas fa-edit"></i>
                </button>
                <button onclick="hapusSiswa(<?= (int)$siswa['id'] ?>, '<?= htmlspecialchars(addslashes($siswa['nama'])) ?>')"
...
<div id="add-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
  <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4">
    <h3 class="text-lg font-bold text-gray-800 mb-4">Tambah Siswa</h3>
    <div class="mb-4">
    ...
    <div class="mb-4">
      ...
      <input type="text" id="add-nisn" placeholder="Masukkan 10 digit NISN" maxlength="10"
        class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
    </div>
    <div class="mb-4">
      <label class="block text-sm font-medium text-gray-700 mb-1">Kelas</label>
      <select id="add-kelas" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        <option value="">— Tidak ada kelas —</option>
        <?php foreach ($kelasList as $kelas): ?>
        <option value="<?= htmlspecialchars($kelas['id']) ?>">
          <?= htmlspecialchars($kelas['nama_kelas']) ?> (<?= htmlspecialchars($kelas['id']) ?>)
        </option>
        <?php endforeach; ?>
      </select>
    </div>
    <div class="mb-5 p-3 bg-blue-50 rounded-lg text-xs text-blue-700">
      ...
      Password default: <strong>123456</strong> (siswa dapat mengubahnya setelah login)
    </div>
...
<div id="edit-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
  <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4">
    <h3 class="text-lg font-bold text-gray-800 mb-4">Edit Siswa</h3>
    ...
    <div class="mb-4">
      <label class="block text-sm font-medium text-gray-700 mb-1">NISN (10 Digit) <span class="text-red-500">*</span></label>
      <input type="text" id="edit-nisn" maxlength="10"
        class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
    </div>
    <div class="mb-5">
      <label class="block text-sm font-medium text-gray-700 mb-1">Kelas</label>
      <select id="edit-kelas" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        <option value="">— Tidak ada kelas —</option>
        <?php foreach ($kelasList as $kelas): ?>
        <option value="<?= htmlspecialchars($kelas['id']) ?>">
          <?= htmlspecialchars($kelas['nama_kelas']) ?> (<?= htmlspecialchars($kelas['id']) ?>)
...
<div id="import-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
  <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4">
    <h3 class="text-lg font-bold text-gray-800 mb-2">Import Siswa dari CSV</h3>
    <p class="text-sm text-gray-500 mb-4">
      Upload file CSV dengan kolom: <strong>Nama Siswa</strong>, <strong>NISN</strong>, <strong>ID Kelas</strong>
    </p>

    <div class="mb-4">
      <a href="siswa.php?action=download_template"
    ...
    <div class="flex gap-3 justify-end">
      <button onclick="closeImportModal()" class="px-4 py-2 border rounded-lg text-sm hover:bg-gray-50">Batal</button>
...
function filterTable() {
  ...
  const rows = Array.from(document.querySelectorAll('.siswa-row'));
...
function openAddModal() {
  ...
  document.getElementById('add-nisn').value  = '';
...
function submitTambah() {
  ...
  const nisn    = document.getElementById('add-nisn').value.trim();
  const kelasId = document.getElementById('add-kelas').value;
  if (!nama) { Swal.fire('Error', 'Nama siswa wajib diisi', 'error'); return; }
  ...
  fd.append('nama', nama);       fd.append('nisn', nisn); fd.append('kelas_id', kelasId);
  postAction(fd, closeAddModal);
}

/* --- Edit modal --- */
function editSiswa(id, nama, nisn, kelasId) {
...
function submitEdit() {
  ...
  fd.append('nisn', nisn);       fd.append('kelas_id', kelasId);
...
function hapusSiswa(id, nama) {
  Swal.fire({
    title: 'Hapus Siswa?',
    text: `"${nama}" akan dihapus permanen beserta data ujiannya.`,
...
function hapusTerpilih() {
  ...
  Swal.fire({
    ...
    text: 'Semua siswa terpilih akan dihapus permanen.',
    icon: 'warning', showCancelButton: true,
...
function resetPasswordMultiple() {
  ...
  Swal.fire({
    ...
    text: 'Password semua siswa terpilih akan direset ke 123456.',
    icon: 'warning', showCancelButton: true,
    confirmButtonColor: '#d97706', confirmButtonText: 'Reset', cancelButtonText: 'Batal'
  }).then(r => {
    if (!r.isConfirmed) return;
...
function submitImport() {
  ...
  fetch('siswa.php', { method: 'POST', body: fd })
    .then(r => r.json())
    .then(data => {
      if (data.success) {
